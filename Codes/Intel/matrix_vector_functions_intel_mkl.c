...
void compact_QR_factorization(mat *M, mat *Q, mat *R){
    ...
    //LAPACKE_dgeqpf(LAPACK_COL_MAJOR, R_full->nrows, R_full->ncols, R_full->d, R_full->nrows, jpvt, tau->d);
...
/* initialize a random matrix for double */
...
void matrix_get_selected_columns_new(mat *M, int *inds, mat *Mc){
    MKL_INT i;
    vec *col_vec = vector_new(M->nrows);
    for(i=0; i<(Mc->ncols); i++){
        matrix_get_col(M,inds[i],col_vec);
